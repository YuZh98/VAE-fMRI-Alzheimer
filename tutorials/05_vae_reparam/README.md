...
A VAE samples a latent `z ~ N(mu, sigma)` and then asks autograd to push
gradients through that sample. Autograd can't: sampling is not a
differentiable function of `mu` and `sigma`. The reparameterization
trick rewrites the sample as `z = mu + sigma * eps` with
`eps ~ N(0, I)`, so the randomness lives in an external noise tensor
and gradients flow through `mu` (additively) and `sigma`
(multiplicatively). The catch in practice is that `eps` needs the same
`device` and `dtype` as `mu`, which is the bug fixed in
`recvae/model.py:182-193` relative to the original notebook.
...
and a learned decoder `p(x | z)`. The marginal `log p(x)` is
intractable, so we maximize a lower bound by introducing an inference
network `q(z | x)`:
...
`z ~ q(z | x)`. To train, we need a gradient of the bound with respect
to the parameters of `q`, including `mu` and `sigma`.
...
`z` is now a deterministic function of `mu`, `sigma`, and a noise
tensor `eps`. Gradients flow through `mu` (additive) and `sigma`
(multiplicative) like through any other op. Only `eps` is
non-differentiable, and we don't need its gradient because it isn't a
parameter.
...
(`log_var` instead of `sigma` directly because it can be any real
number without breaking positivity.)
...
which makes every fresh tensor land on CUDA by default. That global
flag is now deprecated and has several footguns:

- It does not exist on MPS (Apple Silicon).
- It makes every helper script and every third-party library imported
  on this process also default to CUDA, which is hostile to
  composability.
- It silently breaks if no CUDA device is present; the model fails to
  construct, not in `reparametrize`.

The fix at `recvae/model.py:191-192`:
...
1. Implements `reparametrize(mu, log_var)` from scratch (one screen of
   code).
2. Runs it on a tiny `(2, 10)` `mu` and `log_var` and prints the
   result.
...
5. Prints to demonstrate that `torch.randn(2, 10)` lands on CPU by
...
it is a one-liner; for other distributions you may need rejection
sampling or score-function estimators (REINFORCE), which have much
higher variance.
...
- Lesson 10 covers `device`-agnostic code in more depth, including why
  `torch.set_default_tensor_type` is bad practice.
